...
def select_images(image_list, max_images=4, max_days_between=3):
    """Select up to max_images from image_list ensuring no more than max_days_between days apart."""
    selected_images = []
    for i in range(image_list.size().getInfo()):
        image = ee.Image(image_list.get(i))
        date = datetime.utcfromtimestamp(image.date().millis().getInfo() / 1000)

        if not selected_images:
            selected_images.append(image)
        elif len(selected_images) < max_images:
            last_date = datetime.utcfromtimestamp(selected_images[-1].date().millis().getInfo() / 1000)
            if (date - last_date).days <= max_days_between:
                selected_images.append(image)
...
def process_images_from_table(file_path):
    ...
    for index, row in df.iterrows():
        ...
        if num_images > 0:
            ...
            selected_images = select_images(image_list)

            # Export each selected image in both RGB and NBR format
